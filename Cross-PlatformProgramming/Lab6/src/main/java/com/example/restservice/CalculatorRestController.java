...
public class CalculatorRestController {
	...
	public List<String> calculatorPost(@RequestBody @NotNull List<PostParameterClass> list) {
		...
		AtomicLong total = new AtomicLong(), successful = new AtomicLong(), failed = new AtomicLong();
		list.forEach((element) -> {
			try {
				total.incrementAndGet();
				result.add(cache.requestCache(element));
		...
		result.add("Total amount of requests: " + total);
		result.add("Amount of successful requests: " + successful);
		result.add("Amount of failed requests: " + failed);
