...
public final class CalculatorCacheService {
	...
	public @NotNull String requestCache(@NotNull PostParameterClass element) {
		return requestCache(element.units(), String.valueOf(element.value()));
	}
	/**
	 * Service class interface for cache management.
	...
	public @NotNull String requestCache(@NotNull String units, String value) {
		...
		return switch (units) {
			case "meters" -> value + " meter(s) is " + meters2inches(value) + " inch(es).";
			case "inches" -> value + " inch(es) is " + inches2meters(value) + " meter(s).";
			default -> throw new UnitsRecognitionException(units);
		};
	}
}
